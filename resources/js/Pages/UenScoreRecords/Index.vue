<script setup>
  ...
  onMounted(() => {
    ...
    // 初始化搜尋條件
    if (params.has("params.class_no")) {
      searchForm.class_no = params.get("params.class_no");
    }
  ...
  <div class="container mx-auto p-4">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 portrait:grid-cols-2">
        <!-- 搜尋班級 -->
        <el-input v-model="searchForm.class_no" placeholder="搜尋班級" clearable @keyup.enter="handleSearch"
          @clear="handleSearch">
          <template #prefix>
            <el-icon>
              <Search />
        ...
        <!-- 選擇周別 -->
        <el-select v-model="searchForm.week_no" placeholder="選擇周別" clearable @change="handleSearch"
          @clear="handleSearch" class="w-full">
          <template #prefix>
            <el-icon>
              <Search />
            </el-icon>
          ...
          <el-option v-for="option in weekOptions" :key="option.value" :label="option.label" :value="option.value"
            placeholder="選擇周別" clearable @clear="handleSearch" />
        </el-select>

        <!-- 日期範圍選擇 -->
        <el-date-picker v-model="searchForm.date_range" type="daterange" start-placeholder="選擇日期範圍"
          @clear="handleSearch" class="w-full" />
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <!-- 使用分頁組件 -->
      <BasePagination v-model:pagination="pagination" @size-change="handleSizeChange"
