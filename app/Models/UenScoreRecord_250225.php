...
class UenScoreRecord_250225 extends Model {
  ...
  public function scopeWithTargetInfo($query) {
    ...
    return $query
      ...
      ->select([
        ...
        'uen_score_records.target_no',
        // 使用新的視圖別名 ucsc
        // DB::raw('COALESCE(ussc.student_id, ucsc.class_no) as target_name'),
        DB::raw("CASE  WHEN uen_score_records.target_type = 'student' THEN ussc.student_id WHEN uen_score_records.target_type = 'class' THEN ucsc.class_no  ELSE CONCAT('未知類型:', uen_score_records.target_type) END as computed_target_name"),
